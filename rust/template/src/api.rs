...
impl HDDlog {
    ...
    pub fn apply_updates_from_flatbuf(&self, buf: &[u8]) -> Result<(), String>
    {
        ...
        let upds: Result<Vec<Update<Value>>, String> = cmditer.map(|cmd| Update::from_flatbuf(cmd)).collect();
...
#[no_mangle]
pub unsafe extern "C" fn ddlog_apply_updates(prog: *const HDDlog, upds: *const *mut record::UpdCmd, n: libc::size_t) -> raw::c_int
{
    ...
    n: libc::size_t) -> raw::c_int
{
    ...
    let res = prog.apply_updates_from_flatbuf(slice::from_raw_parts(buf, n))
                  .map(|_|0).unwrap_or_else(|e| {
...
pub unsafe extern "C" fn ddlog_apply_updates_from_flatbuf(
    prog: *const HDDlog,
    _buf: *const u8,
    _n: libc::size_t) -> raw::c_int
{
    if prog.is_null() {
        return -1;
    };
    let prog = Arc::from_raw(prog);
    prog.eprintln(&format!("ddlog_apply_updates_from_flatbuf(): error: DDlog was compiled without FlatBuffers support"));
    Arc::into_raw(prog);
    -1
}
