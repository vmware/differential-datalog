...
use timely::logging::{TimelyEvent, StartStop, ParkEvent};
use std::fs::File;
use std::fmt::Debug;
...
impl CSVLogEvent {
    ...
    fn message_entry(worker_id: usize, is_send: bool, channel_id: usize,
                     source: usize, target: usize, data_length: usize) -> CSVLogEvent {
        CSVLogEvent {
            worker_id,
...
impl Statistics {
    ...
    pub fn handle_event(&mut self,timestamp: Duration, worker_index: usize, data: &TimelyEvent,
                        addresses: &FnvHashMap<usize, Vec<usize>>, names: &FnvHashMap<usize, String>) {
            match data {
                TimelyEvent::GuardedMessage(g) => {
                    if g.is_start {
                        ...
                        if let Some(start_time) = self.guarded_message_start_time.pop_start_time(&worker_index) {
                    ...
                    match s.start_stop {
                        ...
                        StartStop::Stop => {
                            ...
                            if let Some(start_time) = self.schedule_start_time.pop_start_time(key) {
                                ...
                                let op_addr = addresses.get(&s.id).expect("Address should have been in map");
                ...
                TimelyEvent::Messages(m) => {
                    let e = CSVLogEvent::message_entry(worker_index, m.is_send, m.channel,
                                                       m.source, m.target, m.length);
                    self.csv_writer.serialize(e).expect("unable to serialize record");
